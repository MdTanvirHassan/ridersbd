...
class ProductController extends Controller
{
    public function query(){
        ...
        $page_title = "All Products Query";
        $empty_message = "No Product Yet";
        $products = Product::join('product_queries','product_queries.product_id', 'products.id')->select('products.*','products.name as pro_name','product_queries.name as p_name','product_queries.*')->orderBy('product_queries.created_at','desc')
                        ->paginate(getPaginate());

        return view('admin.products.query', compact('page_title', 'empty_message', 'products', 'now'));
    }
...
{
    $product = Product_query::find($request->product_id);
    if ($product) {
        $product->status = 1;
        $product->save();
